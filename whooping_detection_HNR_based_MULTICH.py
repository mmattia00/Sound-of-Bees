...
def plot_hnr_analysis(results, ch_num, peaks=None, peak_windows=None, threshold=None):
...
def save_whoop_candidate(segment, sr, audio_filename, channel, peak_time, start_time, end_time, output_dir="whooping_candidates"):
    """
    ...
    channel : int
        Numero del canale
    peak_time : float
        Tempo centrale del picco in secondi
    start_time : float
        Inizio della finestra in secondi
    end_time : float
        Fine della finestra in secondi
    output_dir : str
    ...
    for file_idx, audio_filename in enumerate(audio_files, 1):
        ...
        for j in range(signal_multichannel.shape[1]):
            ...
            try:
                ...
                # Merge finestre
                peak_windows, peaks, peak_times = merge_overlapping_windows(
                    peak_windows, peaks, peak_times
                )
                
                print("-> " + str(len(peak_windows)) + " candidati")
