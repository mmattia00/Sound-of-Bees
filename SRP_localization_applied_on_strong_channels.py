...
def preprocess_signal(signal, sr, lowpass_cutoff=15000, highpass_cutoff=2500):
    """
...
def extract_and_play_channels(data, samplerate, start_time, end_time, 
                               num_channels=16, pause_between=0.5, channel_broken=None, lowpass_cutoff=20000, highpass_cutoff=2000):
    """
    ...
    for ch in range(num_channels):
        if ch in channel_broken:
            ...
            # preprocess
            tmp = preprocess_signal(tmp, samplerate, lowpass_cutoff, highpass_cutoff)
...
def main():
    ...
    # test con whoop significativi
    # starting_whoop = "audio_recording_2025-09-15T06_40_43.498109Z_ch04_5.005s_3.505-6.505s.wav"
    # starting_whoop = "audio_recording_2025-09-15T07_18_43.462247Z_ch12_47.135s_45.635-49.295s.wav"
    starting_whoop = "audio_recording_2025-09-15T08_04_43.560131Z_ch06_44.705s_43.205-46.205s.wav"
    ...
    for file_idx, whoop_filename in enumerate(whoops, 1):
        ...
        # # Plot identico al tuo
        # plt.figure(figsize=(10, 5))
        # plt.bar(range(1, detector.num_channels + 1), channel_levels, color='skyblue')
        # plt.xlabel('Canale')
        # plt.ylabel('HNR Level')
        # plt.title('Livelli HNR dei Canali con Rilevamento Whoop')
        # plt.xticks(range(1, detector.num_channels + 1))
        # plt.ylim(0, max(channel_levels) + 5)  # Se max > 0, altrimenti errore
        # if max(channel_levels) > 0:
        #     plt.ylim(0, max(channel_levels) * 1.1)
        # plt.grid(axis='y')
        # plt.show()
        ...
        # usando solo i canali che hanno effettivamente il whoop rilevato per la localizzazione SRP
        validated_channels = [ch.channel_num for ch in results['channels_with_whoop']]

        mic_array = MicrophoneArray(
                    positions=channel_coords[:num_channels_default],
                    sample_rate=sr,
        ...
        grid = localizer.create_search_grid_centered_on_channel(x_width=0.1, y_height=0.1, resolution=0.005, reference_channel=results['strongest_channel'].channel_num)

        print(f"Using search window: {results['search_window'].time_min:.3f}s - {results['search_window'].time_max:.3f}s")


        signals = signal_multichannel[int(results['search_window'].time_min*sr):int(results['search_window'].time_max*sr), :num_channels_default]
        # 4. Localizza!
