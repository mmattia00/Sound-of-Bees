...
class WhoopFeatures:
    ...
    harmonics_energies: Optional[np.ndarray] = None  # magnitudes delle armoniche
...
class PipelineDraft:
    ...
    def compute_spectrogram(self, plot: bool = False):
            
        S = librosa.stft(self.whoop_audio)
        self.features.spectrogram_dB = librosa.amplitude_to_db(np.abs(S))
        
        if plot:
            plt.figure(figsize=(10, 4))
            librosa.display.specshow(self.features.spectrogram_dB, sr=self.sr, x_axis='time', y_axis='hz')
            plt.colorbar(format='%+2.0f dB')
            plt.title(f"Whoop Spectrogram")
            plt.show()
    ...
    def extract_pitch_features(self, plot: bool = False, verbose: bool = False) -> Dict[str, Any]:
        ...
        f0, _, _, whoop_pitch_info = detector.estimate_f0(
            length_queue=5, hz_threshold=25, threshold_increment=1.3,
            plot=plot, padding_start_ms=5, padding_end_ms=25,
        ...
        return whoop_pitch_info

    def extract_harmonics_features(self, f0_median_sample: int, verbose: bool = False, plot: bool = False):
        ...
        harmonics_info = harmonic_analyzer.analyze_harmonics(
            ...
            f0=self.features.f0_mean,
            center_sample=f0_median_sample,
        ...
        if harmonics_info is not None:
            ...
            # print("harmonics magnitudes:", harmonics_info['harmonic_energies'])

            self.features.harmonics_energies = harmonics_info['harmonic_energies']

    def strongest_channel_detection(self, plot: bool = False, verbose: bool = False, listening_test: bool = False):
        """Placeholder per future estrazione di feature del canale più forte."""
        # Crea detector e analizza
        detector = StrongChannelDetector(self.multichannel_audio, self.sr, self.features.ch, self.features.start_peak,
                                          self.features.end_peak, self.features.f0_mean, broken_channels=self.channel_broken, 
                                          num_channels=16, verbose=verbose,plot=False)
        if listening_test:
            detector.listening_test(pause_between=1.0)

        results = detector.detect_strong_channels(detector_percentile=65)
    ...
    def run_full_pipeline(self, plot: bool = False, verbose: bool = False, listening_test: bool = False) -> WhoopFeatures:
        ...
        self.compute_spectrogram(plot=plot)      

        # estrai feature di pitch
        whoop_pitch_info = self.extract_pitch_features(plot=plot)
        ...
        if whoop_pitch_info is not None:
            ...
            self.extract_harmonics_features(
                f0_median_sample=whoop_pitch_info['f0_median_sample'],
                verbose=verbose,
                plot=plot
            )
            ...
            self.strongest_channel_detection(plot=plot, verbose=verbose, listening_test=listening_test)
            ...
            if self.features.num_channels_with_whoop is not None and self.features.num_channels_with_whoop >= 6:
                ...
                self.precise_whoop_localization(validated_channels, verbose=verbose, plot=plot)
            else:
    ...
    def save_features_to_database(self, hdf5_path: str = 'whoop_database.h5'):
        ...
        group_name = self.whoop_candidate_filename.replace('.wav', '')
        ...
        with h5py.File(hdf5_path, 'a') as f:
            ...
            # Strong channel ( -1 se None)
            grp.create_dataset('strongest_channel', data=np.array(self.features.strongest_channel) if self.features.strongest_channel is not None else np.array(-1, dtype=int))
            grp.create_dataset('num_channels_with_whoop', data=np.array(self.features.num_channels_with_whoop) if self.features.num_channels_with_whoop is not None else np.array(-1, dtype=int))
            
            # ===== ARRAY PICCOLI (senza compressione) =====
            if self.features.harmonics_energies is not None:
                grp.create_dataset('harmonics_energies', data=np.array(self.features.harmonics_energies))  # ~10 valori
            ...
            # ===== ARRAY GRANDE (SOLO spectrogram con compressione) =====
