...
def start_jack_play_and_route():
    """
    Avvia jack-play sul file mono SYNC_WAV in background
    e collega tutti i port jack-play-*:out_1 al canale 10 di master e slave.
    Ritorna il Popen del processo jack-play (per poterlo killare alla fine).
    """
    ...
    # collega tutti i jack-play-*:out_1 trovati
    for src in ports_out1:
        safe_connect(src, "system:playback_10")
        safe_connect(src, "slave_in:playback_10")
...
def main():
    ...
    parser.add_argument(
        "-c", "--channels", type=int, default=20,
        help="Number of channels to record (default: 20 = 10 system + 10 slave_in)"
    )
    ...
    print("Using ports: system:capture_* + slave_in:capture_*")
