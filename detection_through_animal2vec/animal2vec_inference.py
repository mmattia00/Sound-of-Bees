        ...
        for samples in tqdm(dataloader, total=len(dataset.fnames),
            ...
            with embedding_context as f_h5:
                ...
                for bi, batch in enumerate(tqdm(batched_wav, desc="Inference for {} batch(es) of {} "
                    ...
                    # DEBUG
                    # _probs = torch.sigmoid(net_output["encoder_out"].detach().cpu()).float()
                    # tqdm.write(f"[DEBUG] probs mean: {_probs.mean():.4f}, max: {_probs.max():.4f}")
                    ...
                    fused_preds = model.fuse_predict(batch.size(-1), probs,
                                                     ...
                                                     multiplier=bi,
                                                     bs=args.batch_size)
                    # tqdm.write(f"[DEBUG] fused_preds intervals: {fused_preds[1]}")
                    # tqdm.write(f"[DEBUG] fused_preds likelihoods: {fused_preds[0]}")
            ...
            with file_context_one as f, file_context_two as f_sec:  # open file for writing
                ...
                for ti, lik in tqdm(zip(time_intervals, likelihoods),
                    ...
                    # --- PATCH: handle case where 'focal' is not the last class ---
                    # Original:  
                    # starts = flatten([[to_td_from_sec(x[0]) for x in y] for y in ti[:-1]])
                    # ...
                    # likes  = flatten(lik[:-1])

                    has_focal = unique_labels[-1] == "focal"
                    ti_nf   = ti[:-1]  if has_focal else ti
                    lik_nf  = lik[:-1] if has_focal else lik
                    # ---
                    
                    # we exclude the last prediction, as this is the focal predictor
                    starts = flatten([[to_td_from_sec(x[0]) for x in y] for y in ti_nf])
                    durations = flatten([[to_td_from_sec(x[1] - x[0]) for x in y] for y in ti_nf])
                    names = flatten([[unique_labels[ii] for _ in y] for ii, y in enumerate(ti_nf)])
                    likes = flatten(lik_nf)
                    ...
                    # PATCH: handle case where 'focal' is not the last class
                    # ORIGINALE: leggeva sempre ti[-1] come focal
                    # focal_starts    = [to_td_from_sec(x[0]) for x in ti[-1]]
                    # focal_durations = [to_td_from_sec(x[1] - x[0]) for x in ti[-1]]
                    focal_starts    = [to_td_from_sec(x[0])       for x in ti[-1]] if has_focal else []
                    focal_durations = [to_td_from_sec(x[1] - x[0]) for x in ti[-1]] if has_focal else []
                        ...
                        if du > to_td_from_sec(args.min_pred_length) and (
                            ...
                            if args.write_non_focal and foc == 0:
                                if write_secondary_pred_file and not is_highest_likelihood:
                                    ...
                                    # PATCH: considering the case where we don't have focal
                                    # ORIGINALE: appendeva sempre " nf" al nome
                                    # read_out_line = [n + " nf", str(sa), str(du), "decimal", "Cue", desc]
                                    name_out = n + " nf" if has_focal else n
                                    read_out_line = [name_out, str(sa), str(du), "decimal", "Cue", desc]
                                    write_out_lines.update({str(sa) + "_" + n: read_out_line})
