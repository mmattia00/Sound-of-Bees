...
# NEW: fallback splitter for single-class datasets where multilabel stratification is not applicable.
...
def main(args):
    ...
    # BEFORE:
    #   - We always initialized MultilabelStratifiedShuffleSplit when valid_percent > 0.
    #   - This crashed for single-class targets (shape Nx1), because iterstrat expects
    #     a multilabel-indicator target (typically >= 2 label columns).
    # AFTER:
    #   - We enter split setup only when labeled samples exist.
    #   - If there is only one class column, we fallback to ShuffleSplit.
    #   - If valid_percent == 0, we emit a single split with all labeled files in train.
    if len(labels_X) > 0 and len(targets) > 0:
        if not args.valid_set_individuals:
            if 0 < args.valid_percent:
                ...
                # NEW: use iterstrat only when target matrix is truly multilabel (N x C, C > 1).
                if target_matrix.ndim == 2 and target_matrix.shape[1] > 1:
                    sss = MultilabelStratifiedShuffleSplit(
                        n_splits=args.n_split,
                        test_size=args.valid_percent,
                        random_state=args.seed,
                    )
                    it_ = sss.split(labels_X, target_matrix)
                else:
                    # NEW: safe fallback for binary/single-class case.
                    print("[main] STEP 6: only one label class found; using random ShuffleSplit instead of multilabel stratification.")
                    ss = ShuffleSplit(
                        n_splits=args.n_split,
                        test_size=args.valid_percent,
                        random_state=args.seed,
                    )
                    it_ = ss.split(labels_X)
            else:
                # NEW: no validation split requested.
                it_ = iter([(np.arange(len(labels_X)), np.array([], dtype=int))])
        else:
            # Existing behavior: custom split based on individual IDs.
