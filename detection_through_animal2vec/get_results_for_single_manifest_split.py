    ...
    if args.export_predictions or args.export_embeddings:
        with torch.inference_mode(), embedding_context as f_h5, prediction_context as f_pred_h5:
            ...
            for samples in tqdm(dataloader,
                ...
                # print(targets.size())
                # source = samples["source"].squeeze() OLD
                source = samples["source"].squeeze(1)  # rimuove solo la dim 1, non il batch
                # print(source.size())
...
# Example usage:
# python3 get_results_for_single_manifest_split.py --model_path /abyss/home/mattia-montanari/runs/bee-finetune-last-checkpoint-no-augmentation/checkpoints/checkpoint_best.pt --manifest_path /local/labelled_dataset_no_augmentation/preprocessed_audios/Bees_10s_2026-06-15/manifest --unique_labels "['stop_signal_candidate']" --sample_rate 24000 --conv_feature_layers '[(127, 63, 1)] +[(512, 10, 5)] + [(512, 3, 4)] + [(512, 3, 3)] + [(512, 3, 2)] + [(512, 3, 1)] + [(512, 2, 1)] * 2' --out_path /abyss/home/timon-nas/a2v_estimates/ --export_predictions True --min_label_size 0