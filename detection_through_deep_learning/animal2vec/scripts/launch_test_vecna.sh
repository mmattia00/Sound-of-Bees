...
mkdir -p "$TMPDIR"
mkdir -p /abyss/home/checkpoints/bee-pretrain-vecna-4gpu
mkdir -p /abyss/home/tb_logs/bee-pretrain-vecna-4gpu

echo "[INFO] Avvio training con config: $CONFIG_DIR/test_pretrain"
echo "[INFO] Checkpoints -> /abyss/home/checkpoints/bee-pretrain-vecna-4gpu"
echo "[INFO] TensorBoard -> /abyss/home/tb_logs/bee-pretrain-vecna-4gpu"
echo "[INFO] Timestamp avvio: $(date)"

python "$TRAIN_SCRIPT" \
    --config-dir="$CONFIG_DIR" \
    --config-name=test_pretrain_vecna
